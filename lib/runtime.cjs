'use strict';

const crypto = require('node:crypto');
const execFileSync = require('node:child_process').execFileSync;
const fs = require('node:fs');
const Module = require('node:module');
const path = require('node:path');
const vm = require('node:vm');

function assertFile(filename, description) {
  if (!fs.existsSync(filename)) {
    throw new Error(`[pokeclicker-automation] runtime: ${description} not found: ${filename}`);
  }
}

function canonicalModulePath(filename) {
  return path.normalize(filename).replace(/\.ts$/, '');
}

function defaultImport(value) {
  return { __esModule: true, default: value };
}

function loadTypeScript(gameDir) {
  const typescriptPath = path.join(gameDir, 'node_modules', 'typescript');
  assertFile(path.join(typescriptPath, 'package.json'), 'The game TypeScript dependency');
  return require(typescriptPath);
}

function installTypeScriptLoader(gameDir, mocks = new Map()) {
  const ts = loadTypeScript(gameDir);
  const loaded = new Set();
  const oldTsLoader = Module._extensions['.ts'];
  const oldLoad = Module._load;

  Module._extensions['.ts'] = (module, filename) => {
    const source = fs.readFileSync(filename, 'utf8');
    const output = ts.transpileModule(source, {
      fileName: filename,
      compilerOptions: {
        target: ts.ScriptTarget.ES2020,
        module: ts.ModuleKind.CommonJS,
        esModuleInterop: true,
        importsNotUsedAsValues: ts.ImportsNotUsedAsValues.Remove,
      },
    });
    loaded.add(path.relative(gameDir, filename));
    module._compile(output.outputText, filename);
  };

  Module._load = function loadWithMocks(request, parent, isMain) {
    if (parent && request.startsWith('.')) {
      const requestedPath = canonicalModulePath(path.resolve(path.dirname(parent.filename), request));
      if (mocks.has(requestedPath)) {
        return mocks.get(requestedPath);
      }
    }
    return oldLoad.call(this, request, parent, isMain);
  };

  return {
    loaded,
    restore() {
      Module._load = oldLoad;
      if (oldTsLoader) {
        Module._extensions['.ts'] = oldTsLoader;
      } else {
        delete Module._extensions['.ts'];
      }
    },
  };
}

function evaluateScope(filename, exportedNames) {
  assertFile(filename, 'JavaScript source');
  const source = fs.readFileSync(filename, 'utf8');
  const exports = exportedNames.join(', ');
  return vm.runInThisContext(`(() => {\n${source}\nreturn { ${exports} };\n})()`, { filename });
}

function runPayload(source, globals, resultExpression, filename) {
  const context = vm.createContext({ console, ...globals });
  const value = vm.runInContext(`(() => {\n${source}\nreturn (${resultExpression});\n})()`, context, {
    filename,
  });
  return { context, value };
}

function evaluateTypeScriptScripts(gameDir, filenames, globals, resultExpression) {
  const ts = loadTypeScript(gameDir);
  const paths = filenames.map((filename) => path.join(gameDir, filename));
  paths.forEach((filename) => assertFile(filename, 'Official TypeScript source'));
  const source = paths.map((filename) => fs.readFileSync(filename, 'utf8')).join('\n');
  const output = ts.transpileModule(source, {
    fileName: paths.at(-1),
    compilerOptions: {
      target: ts.ScriptTarget.ES2020,
      module: ts.ModuleKind.None,
    },
  });
  return runPayload(output.outputText, globals, resultExpression, paths.at(-1));
}

function bundleStackFrame(frame, bundleName, ranges) {
  const head = frame.lastIndexOf(bundleName + ':');
  if (head < 0) return frame;
  const rest = frame.slice(head + bundleName.length + 1);
  const match = /^(\d+)(?::(\d+))?/.exec(rest);
  if (!match) return frame;
  const range = ranges.find(({ start, end }) => +match[1] >= start && +match[1] <= end);
  if (!range) return frame;
  const column = match[2] === undefined ? '' : ':' + match[2];
  return frame.slice(0, head) + range.filename + ':' + (+match[1] - range.start + 1) + column + rest.slice(match[0].length);
}

function evaluateScripts(filenames, globals, resultExpression, scriptName = filenames.at(-1)) {
  // scriptName labels the bundle in stack traces. Throws during evaluation are
  // remapped to exact per-file locations below; throws from later calls name the
  // bundle with bundle-offset lines.
  for (const filename of filenames) {
    assertFile(filename, 'JavaScript source');
  }
  // One function scope like the production payload (see inject.cjs): top-level
  // declarations stay mutually hoisted across files. The offset table maps the
  // bundle back to per-file locations for stack traces.
  const bundle = ['(() => {'];
  const ranges = [];
  for (const filename of filenames) {
    const start = bundle.length + 1;
    const fileLines = fs.readFileSync(filename, 'utf8').split('\n');
    ranges.push({ filename, start, end: start + fileLines.length - 1 });
    bundle.push(...fileLines);
  }
  bundle.push('return (' + resultExpression + ');', '})()');
  const context = vm.createContext({ console, ...globals });
  try {
    const value = vm.runInContext(bundle.join('\n'), context, { filename: scriptName });
    return { context, value };
  } catch (error) {
    if (error && typeof error.stack === 'string') {
      error.stack = error.stack.split('\n').map((frame) => bundleStackFrame(frame, scriptName, ranges)).join('\n');
    }
    throw error;
  }
}

function resolveGameDir(projectDir, configuredDir = process.env.POKECLICKER_DIR) {
  return path.resolve(configuredDir || path.join(projectDir, '..', '..', 'pokeclicker', 'pokeclicker'));
}

function createOfficialRandom(Rand, SeededRand, initialSeed) {
  const originalNext = Rand.next;
  Rand.next = () => SeededRand.next();
  SeededRand.seed(initialSeed);

  return {
    seed(value) {
      SeededRand.seed(value);
    },
    restore() {
      Rand.next = originalNext;
    },
  };
}

function createDeepConstant() {
  const proxy = new Proxy(() => {}, {
    get: () => proxy,
    apply: () => undefined,
  });
  return proxy;
}

function sha256(filename) {
  return crypto.createHash('sha256').update(fs.readFileSync(filename)).digest('hex');
}

function readGitOutput(repositoryDir, args) {
  try {
    return execFileSync('git', ['-C', repositoryDir, ...args], {
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'ignore'],
    });
  } catch (_error) {
    return null;
  }
}

function gitRevision(repositoryDir) {
  return readGitOutput(repositoryDir, ['rev-parse', '--short=12', 'HEAD'])?.trim() ?? null;
}

function gitWorktreeDirty(repositoryDir) {
  const output = readGitOutput(repositoryDir, ['status', '--porcelain']);
  return output === null ? null : output.length > 0;
}

module.exports = {
  assertFile,
  canonicalModulePath,
  createDeepConstant,
  createOfficialRandom,
  defaultImport,
  evaluateScope,
  evaluateScripts,
  evaluateTypeScriptScripts,
  gitRevision,
  gitWorktreeDirty,
  installTypeScriptLoader,
  resolveGameDir,
  sha256,
};
