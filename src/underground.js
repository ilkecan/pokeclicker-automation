...
const automateUnderground = (() => {
  function isNeighbouringItemTile(tile, other) {
  ...
  function getCandidateTiles(partitionedTiles) {
    return partitionedTiles.buried.filter((tile) => partitionedTiles.mined.some((other) => isNeighbouringItemTile(tile, other)));
  }

  function digOnce(mine, itemAnchors, itemTilesPartitioned, candidateTilesToDig) {
    ...
    if (hammer.canUseTool()) {
      ...
      const candidateCoordinates = allCandidateTiles.map((tile) => tileGlobalCoordinate(itemAnchors, tile));
    ...
    if (chisel.canUseTool()) {
      ...
      if (itemId === undefined) {
        ...
        const tile = candidateTiles.sort((a, b) => chiselComparisonValue(a) - chiselComparisonValue(b))[0];
        coordinate = tileGlobalCoordinate(itemAnchors, tile);
      }
  ...
  function digMine(mine) {
    ...
    const candidateTilesToDig = new Map(itemTilesPartitioned.entries().map(([itemId, partitionedTiles]) => [itemId, getCandidateTiles(partitionedTiles)]));
    ...
    for (const [itemId, partitionedTiles] of itemTilesPartitioned) {
      for (const tile of partitionedTiles.buried) {
        ko.when(() => tile.layerDepth === 0, () => {
          ...
          candidateTilesToDig.set(itemId, getCandidateTiles(partitionedTiles));
        });
      }
    }
    ...
    return App.game.statistics.secondsPlayed.subscribe(() => digOnce(mine, itemAnchors, itemTilesPartitioned, candidateTilesToDig));
  }
